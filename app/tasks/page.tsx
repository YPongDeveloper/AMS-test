    ...
    <Page>
      ...
      {API_CONFIGURED && loading && (
        <div className="py-16 text-center text-gray-400 animate-pulse">
          {t("กำลังตรวจสอบตัวตน...", "Checking identity...")}
        </div>
      )}
      ...
      {API_CONFIGURED && !loading && serverDown && (
        <div className="py-16 text-center max-w-md mx-auto">
          ...
          <p className="text-sm text-gray-500 mt-2">
            {t(
              "เซิร์ฟเวอร์แพลนฟรีจะหลับเมื่อไม่มีการใช้งานสักพัก — ตัวตน LINE ของคุณพร้อมอยู่แล้ว แค่รอสักครู่แล้วกดลองใหม่",
              "Free-tier servers sleep when idle — your LINE identity is ready, just retry in a moment",
            )}
          </p>
          <button
            onClick={retry}
        ...
        <div className="max-w-5xl mx-auto">
          ...
          <p className="text-xs text-gray-500 mb-4">
            ...
            <span className={`inline-flex items-center gap-1 ${wsOn ? "text-emerald-600" : "text-gray-400"}`}>
              <Radio size={12} /> {wsOn ? t("Realtime เชื่อมต่อแล้ว", "Realtime connected") : t("Realtime หลุด กำลังเชื่อมใหม่", "Reconnecting")}
            </span>
          </p>

          {notice && (
            <div className="mb-4 rounded-lg border border-emerald-200 bg-emerald-50 text-emerald-800 text-sm px-4 py-2.5 flex items-center gap-2">
              <span className="w-2 h-2 rounded-full bg-emerald-500 inline-block" /> {notice}
            </div>
          )}
          {err && (
